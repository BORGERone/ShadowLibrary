...
2. **アプリケーションの起動**
   ...
   ```
   その後、ブラウザで http://127.0.0.1:1337 を開く

3. **ゲームのロック解除**
   - App ID を入力（例：2486820）
   - 必要な設定を選択
   - 「ロック解除」をクリック
...
- **バックアップの作成**：設定ファイルを変更する前に、Steam の設定をバックアップしてください
- **合法性**：お住まいの地域の法律を遵守し、所有しているゲームのみをロック解除してください
- **セキュリティ**：信頼できないソースからマニフェストをダウンロードしないでください
